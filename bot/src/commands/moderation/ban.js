...
module.exports = {
	...
	run: async function run(bitch, message, args) {
		...
		if (
			!(
				message.member.permission.has('banMembers') ||
				message.member.roles.some(roles =>
					guild.automoderator.ignored_roles.includes(roles)
				)
			)
		) {
			return message.channel.createMessage({
				embed: {
					...
					description:
						'You not have `Ban Members` permissions or moderator role.',
					color: c
				}
			});
		}
		...
		if (
			member.roles.some(roles => guild.automoderator.ignored.includes(roles))
		) {
			return message.channel.createMessage({
				embed: {
					author: {
			...
			if (!message.member.highestRole.higherThan(member.highestRole)) {
				return message.channel.createMessage({
					embed: {
						...
						description:
							'You cannot sanction a member with a higher role than yours.',
						color: c
					}
				});
			}
			...
			if (message.member.highestRole.position >= member.highestRole.position) {
				return message.channel.createMessage({
					embed: {
						...
						description:
							'You cannot sanction a member with a higher or equal role than yours.',
						color: c
					}
				});
			}
		...
		if (member.highestRole.higherThan(message.guild.me.highestRole)) {
			return message.channel.createMessage({
				embed: {
					...
					description:
						'My role is lower than that of this user, please put him on top of all the roles.',
					color: c
				}
			});
		}
		...
		if (member.highestRole.position === message.guild.me.highestRole.position) {
			return message.channel.createMessage({
				embed: {
					...
					description:
						'My role is equal than that of this user, please put him on top of all the roles.',
					color: c
				}
			});
		}
